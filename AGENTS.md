...
This framework is designed to be **model-agnostic**. You can use it with:
- **Claude Code** (configured via `.claude/`)
- **Gemini / Goose** (configured via `.gemini/`)
- **Other AI Agents** (configured via `.agents/`)

### Changing models
To change the LLM provider or specific models used by the agents, refer to **`docs/a2a/model-config.md`** and use the **`scripts/update-models.sh`** utility.
...
Agents are defined in `.agents/agents/` and invoked via custom slash commands in `.agents/commands/`.
...
This repository is a **Base Framework**. You have two common ways to use it:

#### 1. Template Repository (Fork/Clone)
For new projects, you can fork or clone this repository and build your product directly within it.
...
#### 2. Project Bootstrapping (Initialization Script)
You can use the provided `scripts/bootstrap-project.sh` to initialize the agentic framework into an **existing** codebase. This copies the necessary `.agents` configuration and documentation structure without merging git histories.
...
Agent definitions in `.agents/agents/` include:
- `name` - Agent identifier
- `description` - When to invoke the agent
- `model` - AI model to use
- `color` - UI color coding

Command definitions in `.agents/commands/` contain the slash command expansion text.
